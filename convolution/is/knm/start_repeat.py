...
communication_off_chip += f.ceil(k * n - transmission_zero - transmission_local - transmission_on_chip, bandwidth)
    ...
    for k_serial in range(0,k_loop_times):
        for n_serial in range(0,n_loop_times):
            for m_serial in range(0,m_loop_times):
                ...
                m_serial_next, k_serial_next, n_serial_next = f.next_num_knm(m_serial, k_serial, n_serial, m_loop_times, k_loop_times, n_loop_times)
                m_serial_before, k_serial_before, n_serial_before = f.before_num_knm(m_serial, k_serial, n_serial, m_loop_times, k_loop_times, n_loop_times)
                ...
                if not(m_serial==m_loop_times-1 and k_serial==k_loop_times-1 and n_serial==n_loop_times-1):
                    if G_A_in_use == 0:
                        ...
                        ifmap_img2col0_new = [[(-1,-1,-1,-1) for i in range(k)] for j in range(n)]
                    ifmap_img2col0_new, ifmap_img2col1_new, transmission_zero, transmission_local, transmission_on_chip = \
                    f.transmission_ifmap(FH, FW, OX, OY, IX, IY, stride, n_coordinate, k_coordinate, n_num_next, k_num_next, n, k, ifmap_img2col0_new, ifmap_img2col1_new, ifmap_img2col0_old, ifmap_img2col1_old, C_channel, G_A_in_use, Batchsize, dataflow_M, dataflow_K)
                    for ii in range(n):
                        for jj in range(k):
                ...
                B_buf_tile_trans = f.ceil(k_num_next * n_num_next - transmission_zero - transmission_local - transmission_on_chip, bandwidth)
                ...
                + B_load0 * f.ceil(k_num_next * n_num_next - transmission_zero - transmission_local - transmission_on_chip, bandwidth) + C_load_in0 * partial_sum[m_serial_next][n_serial_next] * f.ceil(m_num_next * n_num_next, bandwidth)\
    ...
    for k_serial in range(0,k_loop_times):
        for n_serial in range(0,n_loop_times):
            for m_serial in range(0,m_loop_times):
                ...
                print("Simulation progress: {}%: ".format(int(counter/counter_total * 100)), end="")
                sys.stdout.flush()
if m_loop_times == 1 and k_loop_times >= 2 and n_loop_times >= 3:
    for k_serial in range(0,k_loop_times):
        for n_serial in range(0,n_loop_times):
            for m_serial in range(0,m_loop_times):
                counter += 1
    ...
    for k_serial in range(0,k_loop_times):
        for n_serial in range(0,n_loop_times):
            for m_serial in range(0,m_loop_times):
                ...
                # 最后一个
                    ...
                    while(on_chip_alter == 0):
                        ...
                        if tracking == 1:
                            ...
                            'G_A0_ptw':str(G_A0_ptw),'G_A1_ptw':str(G_A1_ptw),'G_A0_ptr':str(G_A0_ptr),'G_A1_ptr':str(G_A1_ptr),\
                            'G_C0_ptr':str(G_C0_ptr),'G_C1_ptr':str(G_C1_ptr),'G_C0_ptw':str(G_C0_ptw),'G_C1_ptw':str(G_C1_ptw)})
                elif m_serial==m_loop_times-1 and k_serial!=k_loop_times-1 and n_serial==0:
                    A_load, B_load, C_load_in, C_load_out = 1, 1, 0, 0
                    A_load0, B_load0, C_load_in0, C_load_out0 = 1, 1, 0, 0
                    bw_in_use = 'B'
                    if G_B_in_use == 0:
                ...
                print("Simulation progress: {}%: ".format(int(counter/counter_total * 100)), end="")
                sys.stdout.flush()
if m_loop_times >= 3 and k_loop_times >= 1 and n_loop_times == 1:
    for k_serial in range(0,k_loop_times):
        for n_serial in range(0,n_loop_times):
            for m_serial in range(0,m_loop_times):
                counter += 1
                ...
                # 最后一个
                    ...
                    while(on_chip_alter == 0):
                        ...
                        if tracking == 1:
                            ...
                            'G_A0_ptw':str(G_A0_ptw),'G_A1_ptw':str(G_A1_ptw),'G_A0_ptr':str(G_A0_ptr),'G_A1_ptr':str(G_A1_ptr),\
                            'G_C0_ptr':str(G_C0_ptr),'G_C1_ptr':str(G_C1_ptr),'G_C0_ptw':str(G_C0_ptw),'G_C1_ptw':str(G_C1_ptw)})
                elif m_serial==m_loop_times-1 and n_serial==0:
                    A_load, B_load, C_load_in, C_load_out = 0, 1, 1, 1
                    A_load0, B_load0, C_load_in0, C_load_out0 = 0, 1, 1, 1
                    bw_in_use = 'B'
                    if G_B_in_use == 0:
                    ...
                    while(on_chip_alter == 0):
                        ...
                        if tracking == 1:
                            ...
                            'G_A0_ptw':str(G_A0_ptw),'G_A1_ptw':str(G_A1_ptw),'G_A0_ptr':str(G_A0_ptr),'G_A1_ptr':str(G_A1_ptr),\
                            'G_C0_ptr':str(G_C0_ptr),'G_C1_ptr':str(G_C1_ptr),'G_C0_ptw':str(G_C0_ptw),'G_C1_ptw':str(G_C1_ptw)})
                elif m_serial==0 and n_serial==n_loop_times-1:
                ...
                # 最后一个
                    ...
                    while(on_chip_alter == 0):
                        ...
                        if tracking == 1:
                            ...
                            'G_A0_ptw':str(G_A0_ptw),'G_A1_ptw':str(G_A1_ptw),'G_A0_ptr':str(G_A0_ptr),'G_A1_ptr':str(G_A1_ptr),\
                            'G_C0_ptr':str(G_C0_ptr),'G_C1_ptr':str(G_C1_ptr),'G_C0_ptw':str(G_C0_ptw),'G_C1_ptw':str(G_C1_ptw)})
                elif m_serial==m_loop_times-1 and not(n_serial==n_loop_times-1 and k_serial==k_loop_times-1):
                    A_load, B_load, C_load_in, C_load_out = 1, 1, 1, 1
                    A_load0, B_load0, C_load_in0, C_load_out0 = 1, 1, 1, 1
                    bw_in_use = 'B'
                    if G_B_in_use == 0:
