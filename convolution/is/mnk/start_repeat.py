    ...
    for m_serial in range(0,m_loop_times):
        for n_serial in range(0,n_loop_times):
            for k_serial in range(0,k_loop_times):
                ...
                m_serial_next, k_serial_next, n_serial_next = f.next_num_mnk(m_serial, k_serial, n_serial, m_loop_times, k_loop_times, n_loop_times)
                m_serial_before, k_serial_before, n_serial_before = f.before_num_mnk(m_serial, k_serial, n_serial, m_loop_times, k_loop_times, n_loop_times)
                ...
                # 最后一个
                    ...
                    while(on_chip_alter == 0):
                        ...
                        if tracking == 1:
                            ...
                            'G_A0_ptw':str(G_A0_ptw),'G_A1_ptw':str(G_A1_ptw),'G_A0_ptr':str(G_A0_ptr),'G_A1_ptr':str(G_A1_ptr),\
                            'G_C0_ptr':str(G_C0_ptr),'G_C1_ptr':str(G_C1_ptr),'G_C0_ptw':str(G_C0_ptw),'G_C1_ptw':str(G_C1_ptw)})
                elif m_serial==0 and k_serial==k_loop_times-1 and n_serial!=0 and n_serial!=n_loop_times-1:
